...
/datum/component/decomposition/proc/decompose()
	...
	decomp.visible_message(span_warning("[decomp] покрывается плесенью[produce_ants ? " и муравьями":""]! Мерзость!"))
	qdel(decomp)
	return
...
/datum/component/decomposition/proc/examine(datum/source, mob/user, list/examine_list)
	...
	switch(time_d / original_time)
		...
		if(0.25 to 0.5) // 50% rotten
			examine_list += span_notice("[parent] начинает выглядеть довольно мерзко.")
		if(0 to 0.25) // 75% rotten
			examine_list += span_danger("[parent] едва ли выглядит съедобным.")
