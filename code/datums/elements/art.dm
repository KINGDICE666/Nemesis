...
/datum/element/art/proc/apply_moodlet(atom/source, mob/living/user, impress)
	...
	switch(impress)
		if(GREAT_ART to INFINITY)
			...
			msg = "Какой [pick("шедевр", "chef-d'oeuvre")]. Так [pick("возвышенно", "впечатляюще", "завораживающе", "безупречно")]!"
		if (GOOD_ART to GREAT_ART)
			user.add_mood_event("artgood", /datum/mood_event/artgood)
			msg = "Достойный арт-объект, приятный для взыскательного глаза."
		if (BAD_ART to GOOD_ART)
			user.add_mood_event("artok", /datum/mood_event/artok)
			msg = "Ни плохо, ни хорошо. Достаточно, чтобы называться \"арт-объектом\"."
		if (0 to BAD_ART)
			user.add_mood_event("artbad", /datum/mood_event/artbad)
			msg = "Ого, выглядит паршиво."

	user.visible_message(span_notice("[user] останавливается и внимательно смотрит на [source]."), \
		span_notice("Вы оцениваете [source]... [msg]"))
...
/datum/element/art/proc/appraise(atom/source, mob/user)
	to_chat(user, span_notice("Вы начинаете оценивать [source]..."))
...
/datum/element/art/rev/apply_moodlet(atom/source, mob/living/user, impress)
	...
	if(user.mind?.has_antag_datum(/datum/antagonist/rev))
		user.add_mood_event("artgreat", /datum/mood_event/artgreat)
		msg = "Какой [pick("шедевр", "chef-d'oeuvre")]. Так [pick("подрывно", "революционно", "объединяюще", "эгалитарно")]!"
	else
		user.add_mood_event("artbad", /datum/mood_event/artbad)
		msg = "Ого, выглядит паршиво."

	user.visible_message(span_notice("[user] останавливается, чтобы осмотреть [source]."), \
		span_notice("Вы оцениваете [source], изучая тонкое мастерство пролетариата... [msg]"))
...
/datum/element/art/commoner/apply_moodlet(atom/source, mob/living/user, impress)
	...
	if(!(user.mind.assigned_role.title in haters))
		user.add_mood_event("artgreat", /datum/mood_event/artgreat)
		msg = "Какой [pick("шедевр", "chef-d'oeuvre")]. Так [pick("близко", "по-простому", "правдиво", "настояще")]!"
	else
		user.add_mood_event("artbad", /datum/mood_event/artbad)
		msg = "Ого, выглядит паршиво."
