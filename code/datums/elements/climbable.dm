...
/datum/element/climbable/proc/get_examine_tags(atom/source, mob/user, list/examine_list)
	...
	examine_list["climbable"] = "Похоже, на это можно забраться."
...
/datum/element/climbable/proc/attack_hand(atom/climbed_thing, mob/user)
	...
	for(var/i in climbers)
		...
		structure_climber.visible_message(span_warning("[structure_climber] сбит с [climbed_thing]."), span_warning("Вас сбивают с [climbed_thing]!"), span_hear("Вы слышите крик [structure_climber], а затем глухой удар."))
...
/datum/element/climbable/proc/climb_structure(atom/climbed_thing, mob/living/user, params)
	...
	user.visible_message(span_warning("[user] начинает забираться на [climbed_thing]."), \
								span_notice("Вы начинаете забираться на [climbed_thing]..."))
	...
	if(do_after(user, adjusted_climb_time, climbed_thing))
		...
		if(do_climb(climbed_thing, user, params))
			user.visible_message(span_warning("[user] забирается на [climbed_thing]."), \
								span_notice("Вы забираетесь на [climbed_thing]."))
			...
			to_chat(user, span_warning("Вам не удаётся забраться на [climbed_thing]."))
