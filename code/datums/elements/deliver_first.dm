...
/datum/element/deliver_first/proc/on_examine(obj/structure/closet/target, mob/user, list/examine_list)
	SIGNAL_HANDLER
	examine_list += span_warning("Электронный доставочный замок не даст открыть это, пока оно не прибудет в пункт назначения: [GLOB.areas_by_type[goal_area_type]].")
	examine_list += span_warning("Этот ящик нельзя продать, пока он не будет открыт.")
...
/datum/element/deliver_first/proc/on_emag(obj/structure/closet/target, mob/emagger)
	SIGNAL_HANDLER
	emagger.balloon_alert(emagger, "замок доставки обойдён")
	remove_lock(target)
...
/datum/element/deliver_first/proc/on_pre_open(obj/structure/closet/target, mob/living/user, force)
	...
	if(istype(target, /obj/structure/closet/crate))
		...
		if(opening_crate.manifest) //we don't want to send feedback if they're just tearing off the manifest
			return BLOCK_OPEN
	if(user)
		target.balloon_alert(user, "доступ закрыт до доставки!")
...
/datum/element/deliver_first/proc/remove_lock(obj/structure/closet/target)
	target.visible_message(span_notice("Доставочный замок [target] самоуничтожается, выбрасывая искры из механизма!"))
