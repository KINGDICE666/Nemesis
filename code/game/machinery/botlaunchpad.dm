/obj/machinery/botpad
	name = "площадка запуска ботов"
	desc = "Облегченная версия орбитальной мех-площадки, модифицированная для запуска ботов. Для работы требует привязки к пульту."
...
/obj/machinery/botpad/proc/launch(mob/living/user)
	...
	for(var/mob/living/robot in get_turf(src))
		if(!isbot(robot))
			user.balloon_alert(user, "на площадке неопознанная форма жизни!")
			return
		if(!isnull(possible_bot))
			user.balloon_alert(user, "на площадке слишком много ботов!")
			return
...
/obj/machinery/botpad/proc/recall(mob/living/user)
	...
	if(isnull(our_bot))
		user.balloon_alert(user, "с площадки не отправляли ботов!")
		return
	user.balloon_alert(user, "бот возвращен на площадку")
