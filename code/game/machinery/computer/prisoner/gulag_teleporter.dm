...
/obj/machinery/computer/prisoner/gulag_teleporter_computer
	name = "консоль телепортера трудового лагеря"
	desc = "Используется для отправки преступников в трудовой лагерь."
	...
	switch(action)
		...
		if("toggle_open")
			if(teleporter.locked)
				to_chat(usr, span_alert("Сначала нужно разблокировать телепортер."))
				return
			teleporter.toggle_open()
			return TRUE
		if("teleporter_lock")
			if(teleporter.state_open)
				to_chat(usr, span_alert("Сначала нужно закрыть телепортер."))
				return
...
/obj/machinery/computer/prisoner/gulag_teleporter_computer/proc/teleport(mob/user)
	if(!contained_id) //incase the ID was removed after the transfer timer was set.
		say("Внимание: невозможно переместить заключенного без действительной ID заключенного!")
		return
	var/id_goal_not_set
	if(!contained_id.goal)
		id_goal_not_set = TRUE
		contained_id.goal = default_goal
		say("Цель ID-карты [contained_id] сброшена к значению по умолчанию: [contained_id.goal] очков.")
	...
	to_chat(prisoner, span_warning("После телепортации у вас слегка кружится голова."))
