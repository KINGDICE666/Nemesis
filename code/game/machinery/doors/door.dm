...
/obj/machinery/door/examine(mob/user)
	...
	if(red_alert_access)
		if(SSsecurity_level.get_current_level_as_number() >= SEC_LEVEL_RED)
			. += span_notice("Из-за угрозы безопасности требования доступа сняты!")
		else
			. += span_notice("При красном коде требования доступа автоматически снимутся.")
	if(has_access_panel)
		. += span_notice("Ее техническая панель [panel_open ? "открыта" : "<b>закручена</b> на месте"].")
...
/obj/machinery/door/proc/check_security_level(datum/source, new_level)
	...
	if(!red_alert_access)
		return
	audible_message(span_notice("[src] жужжит, автоматически снимая требования доступа!"))
...
/obj/machinery/door/proc/attempt_delayed_unres_open(mob/opener)
	...
	balloon_alert(opener, "активируем аварийную защелку...")
...
/obj/machinery/door/proc/stop_pressure_during_unres_open(mob/source)
	...
	balloon_alert(source, "сопротивление давлению!")
	to_chat(source, span_warning("Вы держитесь за аварийную защелку, не давая давлению оттолкнуть вас!"))
...
/obj/machinery/door/proc/crush()
	for(var/turf/checked_turf in locs)
		for(var/mob/living/future_pancake in checked_turf)
			future_pancake.visible_message(span_warning("[src] закрывается на [future_pancake], раздавливая [future_pancake.p_them()]!"), span_userdanger("[src] закрывается на вас и давит!"))
