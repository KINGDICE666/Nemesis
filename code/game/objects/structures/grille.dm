...
/obj/structure/grille/examine(mob/user)
	...
	if(resistance_flags & INDESTRUCTIBLE)
		return
	if(anchored)
		. += span_notice("Она закреплена [EXAMINE_HINT("винтами")]. Похоже, прутья можно [EXAMINE_HINT("разрезать")].")
	else
		. += span_notice("Крепёжные винты [EXAMINE_HINT("откручены")]. Похоже, прутья можно [EXAMINE_HINT("разрезать")].")
		...
		if(RCD_WINDOWGRILLE)
			...
			if(!initial(window_path.fulltile))
				if(!valid_build_direction(loc, user.dir, is_fulltile = FALSE))
					balloon_alert(user, "окно уже здесь!")
					return FALSE
...
/obj/structure/grille/proc/clear_tile(mob/user)
	...
	to_chat(user, span_notice("Вы убираете [unanchored_items_on_tile == 1 ? "[last_item_moved]" : "несколько вещей"] с пути."))

	if(unanchored_items_on_tile - CLEAR_TILE_MOVE_LIMIT > 0)
		to_chat(user, span_warning("На пути всё ещё слишком много вещей!"))
		return FALSE
...
/obj/structure/grille/attack_alien(mob/living/user, list/modifiers)
	...
	user.changeNext_move(CLICK_CD_MELEE)
	user.visible_message(span_warning("[user] кромсает [src]."), null, null, COMBAT_MESSAGE_RANGE)
	if(!shock(user, 70))
...
/obj/structure/grille/screwdriver_act(mob/living/user, obj/item/tool)
	...
	user.visible_message(span_notice("[user] [anchored ? "закрепляет" : "открепляет"] [src]."), \
		span_notice("Вы [anchored ? "закрепляете [src] на" : "открепляете [src] от"] пола."))
	return ITEM_INTERACT_SUCCESS
...
/obj/structure/grille/attackby(obj/item/W, mob/user, list/modifiers, list/attack_modifiers)
	...
	if(istype(W, /obj/item/stack/rods) && broken && do_after(user, 1 SECONDS, target = src))
		...
		user.visible_message(span_notice("[user] восстанавливает сломанную решётку."), \
			span_notice("Вы восстанавливаете сломанную решётку."))
		repair_grille()
		R.use(1)
		return TRUE
...
//window placing begin
		...
		if (!broken)
			...
			if (ST.get_amount() < 2)
				to_chat(user, span_warning("Для этого нужно как минимум два листа стекла!"))
				return
			var/dir_to_set = SOUTHWEST
			if(!anchored)
				to_chat(user, span_warning("Сначала [src] нужно закрепить на полу!"))
				return
			for(var/obj/structure/window/WINDOW in loc)
				to_chat(user, span_warning("Там уже есть окно!"))
				return
			if(!clear_tile(user))
				return
			to_chat(user, span_notice("Вы начинаете устанавливать окно..."))
			if(do_after(user,20, target = src))
				...
				WD.state = 0
				ST.use(2)
				to_chat(user, span_notice("Вы устанавливаете [WD] на [src]."))
			return
//window placing end
