...
/obj/structure/table
	name = "стол"
	desc = "Квадратный кусок железа на четырех металлических ножках. Он не может двигаться."
...
/obj/structure/table/add_context(atom/source, list/context, obj/item/held_item, mob/living/user)
	...
	if(istype(held_item, /obj/item/toy/cards/deck))
		...
		if(HAS_TRAIT(dealer_deck, TRAIT_WIELDED))
			context[SCREENTIP_CONTEXT_LMB] = "Сдать карту"
			context[SCREENTIP_CONTEXT_RMB] = "Сдать карту лицом вверх"
...
/obj/structure/table/examine(mob/user)
	. = ..()
	if(is_flipped)
		. += span_notice("Он опрокинут набок!")
	. += deconstruction_hints(user)

/obj/structure/table/proc/deconstruction_hints(mob/user)
	return span_notice("Столешница прикручена <b>винтами</b>, но основные <b>болты</b> тоже видны.")
...
/obj/structure/table/attack_hand_secondary(mob/user, list/modifiers)
	...
	if(!is_flipped)
		if(!LAZYACCESS(user.do_afters, interaction_key)) // To avoid balloon alert spam
			user.balloon_alert_to_viewers("опрокидывает стол...")
...
/obj/structure/table/wrench_act_secondary(mob/living/user, obj/item/tool)
	if(!deconstruction_ready)
		return NONE
	to_chat(user, span_notice("Вы начинаете демонтировать [src]..."))
...
/obj/structure/table/greyscale/finalize_material_effects(list/materials)
	...
	desc = "Квадратный [(length(materials) > 1) ? "сплав" : "кусок"] материала: [english_list], на четырех ножках. Он не может двигаться."

///Table on wheels
/obj/structure/table/rolling
	name = "передвижной стол"
	desc = "Передвижной стол бренда NT \"Rolly poly\". Он может двигаться и будет двигаться."
	anchored = FALSE
...
/obj/structure/table/rolling/item_interaction(mob/living/user, obj/item/rolling_table_dock/rable, list/modifiers)
	...
	if(rable.loaded)
		to_chat(user, span_warning("У вас уже закреплен \a [rable.loaded]!"))
		return ITEM_INTERACT_FAILURE

	if(locate(/mob/living) in loc.get_all_contents())
		to_chat(user, span_warning("Вы не можете собрать \the [src], пока сверху столько всего!"))
		return ITEM_INTERACT_FAILURE
...
/obj/structure/table/glass
	...
	desc = "Что я говорил про то, чтобы не опираться на стеклянные столы? Теперь вам нужна операция."
...
/obj/structure/table/glass/proc/table_shatter(mob/living/victim)
	visible_message(span_warning("[src] разбивается!"),
...
/obj/structure/table/wood
	...
	desc = "Не подносите к нему огонь. Говорят, он легко горит."
...
/obj/structure/table/wood/after_smash(mob/living/smashed_onto)
	if(QDELETED(src) || prob(66))
		return
	visible_message(
		span_warning("[src] разлетается на щепки!"),
...
/obj/structure/table/wood/fancy
	name = "нарядный стол"
	desc = "Обычный металлический каркас стола, накрытый удивительно нарядной узорчатой тканью."
...
/obj/structure/table/reinforced
	name = "укрепленный стол"
	desc = "Укрепленная версия четырехногого стола."
...
/obj/structure/table/reinforced/deconstruction_hints(mob/user)
	if(deconstruction_ready)
		return span_notice("Верхняя крышка <i>отварена</i>, и <b>болты</b> основного каркаса открыты.")
	else
		return span_notice("Верхняя крышка крепко <b>приварена</b>.")
...
/obj/structure/table/reinforced/welder_act_secondary(mob/living/user, obj/item/tool)
	if(tool.tool_start_check(user, amount = 0))
		...
		if(deconstruction_ready)
			...
			to_chat(user, span_notice("Вы начинаете ослаблять [src]..."))
			if (tool.use_tool(src, user, 50, volume = 50))
...
/obj/structure/table/reinforced/rglass
	name = "укрепленный стеклянный стол"
...
/obj/structure/table/reinforced/plasmarglass
	name = "укрепленный стол из плазменного стекла"
	desc = "Укрепленная версия стола из плазменного стекла."
...
/obj/structure/table/reinforced/titaniumglass
	...
	desc = "Стол из укрепленного титаном стекла со свежим слоем белой краски NT."
...
/obj/structure/table/reinforced/plastitaniumglass
	...
	desc = "Стол из укрепленного титаном кремниево-плазменного композита. Прочен примерно настолько, насколько звучит."
...
/obj/structure/table/optable/add_context(atom/source, list/context, obj/item/held_item, mob/living/user)
	...
	if(breath_mask && breath_mask != held_item)
		...
		context[SCREENTIP_CONTEXT_LMB] = "Подсоединить маску"
		. |= CONTEXTUAL_SCREENTIP_SET
	...
	if(air_tank)
		...
		if (as_tank.tank_holder_icon_state)
			context[SCREENTIP_CONTEXT_LMB] = "Подсоединить баллон"
			. |= CONTEXTUAL_SCREENTIP_SET
...
/obj/structure/table/optable/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if (istype(tool, /obj/item/clothing/mask/breath))
		if (breath_mask && breath_mask != tool)
			balloon_alert(user, "маска уже подсоединена!")
			return ITEM_INTERACT_BLOCKING
		...
		balloon_alert(user, "маска подсоединена")
	...
	if (air_tank)
		balloon_alert(user, "баллон уже подсоединен!")
		return ITEM_INTERACT_BLOCKING
	...
	air_tank = as_tank
	balloon_alert(user, "баллон подсоединен")
	playsound(src, 'sound/machines/click.ogg', 50, TRUE)
	update_appearance()
	return ITEM_INTERACT_SUCCESS
...
/obj/structure/table/optable/wrench_act(mob/living/user, obj/item/tool)
	...
	tool.play_tool_sound(src, 50)
	balloon_alert(user, "баллон отсоединен")
...
/obj/structure/table/optable/examine(mob/user)
	. = ..()
	if (air_tank)
		. += span_notice("К нему прикреплен \a [air_tank] на паре [EXAMINE_HINT("болтов")].")
		if (patient)
			. += span_info("You can connect [patient]'s internals to \the [air_tank] by dragging \the [src] onto them.")
	else
		. += span_notice("Снизу есть крепление для воздушного баллона.")
	if (breath_mask)
		. += span_notice("Сбоку прикреплена \a [breath_mask], трубка закреплена одним [EXAMINE_HINT("винтом")].")
		if (breath_mask.loc == src)
...
/obj/structure/table/optable/proc/detach_mask(mob/living/user)
	...
	if (!breath_mask)
		balloon_alert(user, "маска не подсоединена!")
		return TRUE
	...
	to_chat(user, span_notice("Вы вытаскиваете \the [breath_mask] из \the [src]."))
	update_appearance()
	return TRUE
...
/obj/structure/table/optable/mouse_drop_dragged(atom/over, mob/living/user, src_location, over_location, params)
	...
	if(!iscarbon(patient))
		balloon_alert(user, "нет разъема для дыхания!")
		return

	if (!air_tank)
		balloon_alert(user, "баллон не подсоединен!")
		return
...
/obj/structure/rack
	name = "стеллаж"
	desc = "Не тот, что из Средневековья."
