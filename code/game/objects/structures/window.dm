...
/obj/structure/window/examine(mob/user)
	...
	switch(state)
		...
		if(WINDOW_IN_FRAME)
			. += span_notice("Окно <i>откручено</i>, но <b>вставлено ломом</b> в раму.")
		if(WINDOW_OUT_OF_FRAME)
			if (anchored)
				. += span_notice("Окно <b>прикручено</b> к полу.")
			else
				. += span_notice("Окно <i>откручено</i> от пола, и его можно разобрать <b>ключом</b>.")
...
/obj/structure/window/attack_tk(mob/user)
	...
	user.visible_message(span_notice("Что-то стучит по [src]."))
...
/obj/structure/window/attack_hand(mob/living/user, list/modifiers)
	...
	if(!user.combat_mode)
		...
		user.visible_message(span_warning("[user] бьёт [src]!"), \
			span_warning("Вы бьёте [src]!"))
...
/obj/structure/window/welder_act(mob/living/user, obj/item/tool)
	if(atom_integrity >= max_integrity)
		to_chat(user, span_warning("[src] уже в хорошем состоянии!"))
...
/obj/structure/window/screwdriver_act(mob/living/user, obj/item/tool)
	...
	switch(state)
		if(WINDOW_SCREWED_TO_FRAME)
			to_chat(user, span_notice("Вы начинаете откручивать окно от рамы..."))
			if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				...
				to_chat(user, span_notice("Вы откручиваете окно от рамы."))
		if(WINDOW_IN_FRAME)
			to_chat(user, span_notice("Вы начинаете прикручивать окно к раме..."))
			if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				state = WINDOW_SCREWED_TO_FRAME
				to_chat(user, span_notice("Вы прикручиваете окно к раме."))
		if(WINDOW_OUT_OF_FRAME)
			if(anchored)
				to_chat(user, span_notice("Вы начинаете откручивать раму от пола..."))
				if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
					set_anchored(FALSE)
					to_chat(user, span_notice("Вы откручиваете раму от пола."))
			else
				to_chat(user, span_notice("Вы начинаете прикручивать раму к полу..."))
				if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
					set_anchored(TRUE)
					to_chat(user, span_notice("Вы прикручиваете раму к полу."))
	return ITEM_INTERACT_SUCCESS
...
/obj/structure/window/wrench_act(mob/living/user, obj/item/tool)
	...
	to_chat(user, span_notice("Вы успешно разбираете [src]."))
	qdel(src)
	return ITEM_INTERACT_SUCCESS
...
/obj/structure/window/crowbar_act(mob/living/user, obj/item/tool)
	...
	switch(state)
		if(WINDOW_IN_FRAME)
			to_chat(user, span_notice("Вы начинаете выдавливать окно из рамы..."))
			if(tool.use_tool(src, user, 10 SECONDS, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				...
				to_chat(user, span_notice("Вы выдавливаете окно из рамы."))
		if(WINDOW_OUT_OF_FRAME)
			to_chat(user, span_notice("Вы начинаете вставлять окно обратно в раму..."))
			if(tool.use_tool(src, user, 5 SECONDS, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				state = WINDOW_SCREWED_TO_FRAME
				to_chat(user, span_notice("Вы вставляете окно обратно в раму."))
		else
			return FALSE
...
/obj/structure/window/reinforced
	name = "усиленное окно"
	desc = "Окно, усиленное металлическими прутьями."
	icon_state = "rwindow"
	reinf = TRUE
		...
		if(RWINDOW_SECURE)
			if(tool.tool_behaviour == TOOL_WELDER)
				if(tool.tool_start_check(user, heat_required = HIGH_TEMPERATURE_REQUIRED))
					user.visible_message(span_notice("[user] подносит [tool] к защитным винтам на [src]..."),
						span_notice("Вы начинаете нагревать защитные винты на [src]..."))
					if(tool.use_tool(src, user, 15 SECONDS, volume = 100))
						to_chat(user, span_notice("Защитные винты раскалились добела и выглядят готовыми к снятию."))
						state = RWINDOW_BOLTS_HEATED
						addtimer(CALLBACK(src, PROC_REF(cool_bolts)), 30 SECONDS)
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("Сначала нужно нагреть защитные винты!"))

		if(RWINDOW_BOLTS_HEATED)
			if(tool.tool_behaviour == TOOL_SCREWDRIVER)
				user.visible_message(span_notice("[user] вдавливает инструмент в нагретые защитные винты и начинает их выкручивать..."),
										span_notice("Вы с усилием вдавливаете инструмент в нагретые винты, и они начинают проворачиваться..."))
				if(tool.use_tool(src, user, 50, volume = 50))
					state = RWINDOW_BOLTS_OUT
					to_chat(user, span_notice("Винты выкручены, и по краю панели появляется зазор."))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("Сначала нужно снять защитные винты!"))

		if(RWINDOW_BOLTS_OUT)
			if(tool.tool_behaviour == TOOL_CROWBAR)
				user.visible_message(span_notice("[user] вставляет [tool] в зазор рамы и начинает поддевать..."),
										span_notice("Вы вставляете [tool] в зазор рамы и начинаете поддевать..."))
				if(tool.use_tool(src, user, 40, volume = 50))
					state = RWINDOW_POPPED
					to_chat(user, span_notice("Панель выходит из рамы, открывая тонкие металлические прутья, которые выглядят так, будто их можно перерезать."))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("Сначала нужно поддеть зазор!"))

		if(RWINDOW_POPPED)
			if(tool.tool_behaviour == TOOL_WIRECUTTER)
				user.visible_message(span_notice("[user] начинает резать открытые прутья на [src]..."),
										span_notice("Вы начинаете резать открытые прутья на [src]."))
				if(tool.use_tool(src, user, 20, volume = 50))
					state = RWINDOW_BARS_CUT
					to_chat(user, span_notice("Панели отходят в сторону, открывая болты рамы."))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("Сначала нужно перерезать прутья!"))

		if(RWINDOW_BARS_CUT)
			if(tool.tool_behaviour == TOOL_WRENCH)
				user.visible_message(span_notice("[user] начинает откручивать [src] от рамы..."),
					span_notice("Вы начинаете откручивать болты от рамы..."))
				if(tool.use_tool(src, user, 40, volume = 50))
					to_chat(user, span_notice("Вы откручиваете болты от рамы, и окно освобождается."))
					state = WINDOW_OUT_OF_FRAME
					set_anchored(FALSE)
			else if (tool.tool_behaviour)
...
/obj/structure/window/proc/cool_bolts()
	if(state == RWINDOW_BOLTS_HEATED)
		state = RWINDOW_SECURE
		visible_message(span_notice("Болты на [src], похоже, остыли..."))
...
/obj/structure/window/reinforced/examine(mob/user)
	...
	switch(state)
		if(RWINDOW_SECURE)
			. += span_notice("Оно прикручено односторонними винтами; чтобы их выкрутить, придётся сначала <b>нагреть</b> их.")
		if(RWINDOW_BOLTS_HEATED)
			. += span_notice("Винты раскалены добела, теперь их, скорее всего, можно <b>выкрутить</b>.")
		if(RWINDOW_BOLTS_OUT)
			. += span_notice("Винты сняты, оставив небольшой зазор, куда можно вставить <b>инструмент для поддевания</b>.")
		if(RWINDOW_POPPED)
			. += span_notice("Основная панель окна вышла из рамы, открыв прутья, которые можно <b>перерезать</b>.")
		if(RWINDOW_BARS_CUT)
			. += span_notice("Основную панель можно легко отодвинуть, чтобы открыть <b>болты</b>, удерживающие раму.")
...
/obj/structure/window/plasma
	...
	desc = "Окно из плазменно-силикатного сплава. Выглядит невероятно стойким к ударам и прожиганию."
...
/obj/structure/window/reinforced/plasma
	...
	desc = "Окно из плазменно-силикатного сплава с матрицей из прутьев. Выглядит почти неразрушимым и практически огнеупорным."
...
/obj/structure/window/fulltile
	name = "полнотайловое окно"
	desc = "Окно, занимающее весь тайл."
...
/obj/structure/window/reinforced/fulltile
	name = "полнотайловое усиленное окно"
	desc = "Окно на весь тайл, усиленное металлическими прутьями."
...
/obj/structure/window/reinforced/shuttle//this is called reinforced because it is reinforced w/titanium
	name = "окно шаттла"
	desc = "Усиленное герметичное окно капсулы."
...
/obj/structure/window/reinforced/plasma/plastitanium
	...
	desc = "Прочное на вид окно из сплава плазмы и титана."
...
/obj/structure/window/paperframe
	name = "бумажная перегородка"
	desc = "Хрупкая перегородка из тонкой древесины и бумаги."
...
/obj/structure/window/paperframe/examine(mob/user)
	. = ..()
	if(atom_integrity < max_integrity)
		. += span_info("Выглядит немного повреждённой; возможно, её получится починить <b>бумагой</b>.")
...
/obj/structure/window/paperframe/attackby(obj/item/W, mob/living/user)
	...
	if(istype(W, /obj/item/paper) && atom_integrity < max_integrity)
		user.visible_message(span_notice("[user] начинает заклеивать дыры в [src]."))
		if(do_after(user, 2 SECONDS, target = src))
			atom_integrity = min(atom_integrity+4,max_integrity)
			qdel(W)
			user.visible_message(span_notice("[user] заклеивает часть дыр в [src]."))
			if(atom_integrity == max_integrity)
				update_appearance()
			return
	..()
...
/obj/structure/window/bronze
	name = "латунное окно"
	desc = "Тончайшая пластина полупрозрачной, но усиленной латуни. Хотя нет, это просто хрупкая бронза!"
