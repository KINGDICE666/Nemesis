...
/turf/open/floor/proc/remove_tile(mob/user, silent = FALSE, make_tile = TRUE, force_plating)
	if(broken || burnt)
		...
		burnt = FALSE
		if(user && !silent)
			to_chat(user, span_notice("Вы снимаете повреждённую обшивку."))
	else
		if(user && !silent)
			to_chat(user, span_notice("Вы снимаете напольную плитку."))
		if(make_tile)
			spawn_tile()
	...
	switch(rcd_data[RCD_DESIGN_MODE])
		...
		if(RCD_AIRLOCK)
			...
			if(ispath(airlock_type, /obj/machinery/door/window))
				if(!valid_build_direction(src, user.dir, is_fulltile = FALSE))
					balloon_alert(user, "шлюзовое окно уже здесь!")
					return FALSE
				for(var/obj/machinery/door/door in src)
					if(istype(door, /obj/machinery/door/window))
						continue
					balloon_alert(user, "дверь уже здесь!")
					return FALSE
		...
		if(RCD_DECONSTRUCT)
			if(rcd_proof)
				balloon_alert(user, "слишком толстый!")
				return FALSE
