...
/datum/heretic_knowledge/proc/pre_research(mob/user, datum/antagonist/heretic/our_heretic)
	...
	if(is_final_knowledge && !our_heretic.unlimited_blades)
		var/choice = tgui_alert(user, "ЭТО ОТКЛЮЧИТ ЛОМАНИЕ КЛИНКОВ. Вы готовы изучить это? Ограничение на количество клинков также будет снято.", "Получить финальное заклинание?", list("Да", "Нет"))
		if(choice != "Да")
			return FALSE
	return TRUE
...
/datum/heretic_knowledge/proc/parse_required_item(atom/item_path, number_of_things)
	...
	if(ispath(item_path, /mob/living))
		return "люб[number_of_things > 1 ? "ые" : "ая"] туш[number_of_things > 1 ? "и" : "а"]"
...
/datum/heretic_knowledge/limited_amount/recipe_snowflake_check(mob/living/user, list/atoms, list/selected_atoms, turf/loc)
	...
	if(LAZYLEN(created_items) >= limit)
		loc.balloon_alert(user, "ритуал провален, предел!")
		return FALSE
...
/datum/heretic_knowledge/limited_amount/starting/on_research(mob/user, datum/antagonist/heretic/our_heretic)
	...
	if(!our_heretic.heretic_path)
		...
		to_chat(user, span_warning("Что-то сломалось, путь не найден!"))
...
/datum/heretic_knowledge/knowledge_ritual
	name = "Ритуал знания"
	desc = "Случайно созданный ритуал трансмутации, дающий очки знания. Его можно выполнить только один раз."
	gain_text = "Всё может стать ключом к тайнам за Вратами. Мне нужно быть осторожным и мудрым."
...
/datum/heretic_knowledge/knowledge_ritual/on_research(mob/user, datum/antagonist/heretic/our_heretic)
	...
	to_chat(user, span_hierophant("[name] требует следующее:"))
	...
	to_chat(user, span_hierophant("За завершение вы получите [KNOWLEDGE_RITUAL_POINTS] очка знания. Список требований можно позже посмотреть в изученных знаниях."))

	desc = "Позволяет трансмутировать [english_list(requirements_string)] ради [KNOWLEDGE_RITUAL_POINTS] дополнительных очков знания. Можно выполнить только один раз."
...
/datum/heretic_knowledge/ultimate/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	...
	if(EMERGENCY_IDLE_OR_RECALLED)
		SSshuttle.call_evac_shuttle("Критический разрыв реальности обнаружен дальними сканерами сверхъестественной причинности. Массовые потери экипажа и возможное уничтожение станции признаны недопустимо вероятными. Приоритетный эвакуационный шаттл отправлен.")
