...
/obj/item/codex_cicatrix
	name = "Codex Cicatrix"
	desc = "Этот тяжёлый том полон загадочных каракулей и невозможных диаграмм. \
	По легенде, его можно расшифровать и раскрыть тайны завесы между мирами."
...
/obj/item/codex_cicatrix/Initialize(mapload)
	...
	AddComponent(/datum/component/effect_remover, \
		success_feedback = "Вы удаляете %THEEFFECT.", \
...
/obj/item/codex_cicatrix/examine(mob/user)
	...
	. += span_notice("Можно использовать для сбора влияний ради дополнительных очков знания.")
	. += span_notice("Также позволяет легко рисовать или удалять руны трансмутации.")
	. += span_notice("Кроме того, в руках он работает как фокус для ваших заклинаний.")
...
/obj/item/codex_cicatrix/morbus // I'm morbing all over
	name = "Codex Morbus"
	desc = "Уродливая потрёпанная книга, покрытая глазами, которые моргают каждый сам по себе и все смотрят на вас. Вы не понимаете, как держать эту вещь, и, честно говоря, не уверены, что хотите."
...
/obj/item/codex_cicatrix/morbus/examine(mob/user)
	. = ..()
	if(IS_HERETIC(user))
		. += span_info("Можно использовать для наложения проклятия с кровью во второй руке, кликнув правой кнопкой по руне.")
		return
	. += span_danger("Глаза перестают моргать. Они смотрят на вас. Их взгляд жжёт...")
	if(!ishuman(user))
		return
	var/mob/living/carbon/human/human_user = user
	to_chat(human_user, span_userdanger("Ваш разум горит, пока вы смотрите на страницы!"))
...
/obj/item/codex_cicatrix/morbus/interact_with_atom_secondary(atom/interacting_with, mob/living/user, list/modifiers)
	...
	var/selected_curse = tgui_input_list(user, "Наложить проклятие", "Выберите проклятие!", curse_list, timeout = 0)
	if(!selected_curse)
		return NONE
