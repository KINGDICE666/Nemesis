...
/datum/heretic_knowledge/spell/basic
	name = "Пробуждение Рассвета"
	desc = "Начинает ваше путешествие в Мансус. \
		Дарует Хватку Мансуса, мощное и улучшаемое \
		оглушающее заклинание, которое можно применять без фокуса."
...
/datum/heretic_knowledge/spell/basic/proc/on_grasp_cast(mob/living/carbon/cast_on, datum/action/cooldown/spell/touch/touch_spell)
	...
	cast_on.visible_message(span_notice("[cast_on] щёлкает пальцами рядом с [held_rod], окутывая её вспышкой фиолетового пламени!"))
...
/datum/heretic_knowledge/living_heart
	name = "Живое сердце"
	desc = "Дарует вам Живое сердце, позволяющее отслеживать цели жертвоприношения. \
		Если вы потеряете сердце, вы можете трансмутировать мак и лужу крови, \
		чтобы пробудить своё сердце как Живое сердце. Если ваше сердце кибернетическое, \
		пробудить его не получится."
	required_atoms = list(
...
/datum/heretic_knowledge/living_heart/on_research(mob/user, datum/antagonist/heretic/our_heretic)
	...
	if(!where_to_put_our_heart)
		...
		for(var/backup_slot in backup_organs)
			...
			to_chat(user, span_boldnotice("Так как у вашего вида нет сердца, Живое сердце находится в вашем органе: [look_for_backup.name]."))
			break

	if(where_to_put_our_heart)
		where_to_put_our_heart.AddComponent(/datum/component/living_heart)
		desc = "Дарует Живое сердце, связанное с вашим органом: [where_to_put_our_heart.name]. Оно позволяет отслеживать цели жертвоприношения. \
			Если вы потеряете [where_to_put_our_heart.name], вы можете трансмутировать мак и лужу крови, \
			чтобы пробудить [where_to_put_our_heart.name] как Живое сердце. \
			Кибернетический [where_to_put_our_heart.name] заблокирует ритуал!"

	else
		to_chat(user, span_boldnotice("У вас нет сердца или вообще грудных органов. Поэтому вы не получили Живое сердце."))
...
/datum/heretic_knowledge/living_heart/recipe_snowflake_check(mob/living/user, list/atoms, list/selected_atoms, turf/loc)
	...
	if(QDELETED(our_living_heart))
		loc.balloon_alert(user, "ритуал провален, нет [our_heretic.living_heart_organ_slot]!")
		return FALSE
	...
	if(HAS_TRAIT(our_living_heart, TRAIT_LIVING_HEART))
		loc.balloon_alert(user, "ритуал провален, сердце уже живое!")
		return FALSE
	...
	loc.balloon_alert(user, "ритуал провален, [our_heretic.living_heart_organ_slot] нельзя пробудить!") // "heart can't be awakened!"
	return FALSE
...
/datum/heretic_knowledge/living_heart/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	...
	to_chat(user, span_warning("Вы чувствуете, как [our_new_heart.name] начинает пульсировать всё быстрее, пробуждаясь!"))
...
/datum/heretic_knowledge/amber_focus
	...
	desc = "Позволяет трансмутировать лист стекла и пару глаз, чтобы создать Янтарный фокус. \
		Фокус нужно носить, чтобы применять более продвинутые заклинания."
	required_atoms = list(
...
/datum/heretic_knowledge/spell/cloak_of_shadows
	name = "Плащ Тени"
	desc = "Дарует заклинание Плащ Тени. Оно полностью скрывает вашу личность в фиолетовом дыму \
		на три минуты, помогая сохранить тайну. Для применения требуется фокус."
...
/datum/heretic_knowledge/feast_of_owls
	name = "Пир Сов"
	desc = "Позволяет пройти ритуал, дающий 5 очков знания, но навсегда лишающий вас вознесения. Его можно выполнить только один раз, отменить это нельзя."
	gain_text = "Под мягким сиянием безрассудства скрывается зверь, что крадётся в ночи. Я приведу его сюда и впущу в своё присутствие. Он насытится моими амбициями и оставит знание после себя."
	is_starting_knowledge = TRUE
	required_atoms = list()
...
/datum/heretic_knowledge/feast_of_owls/on_finished_recipe(mob/living/user, list/selected_atoms, turf/loc)
	var/alert = tgui_alert(user,"Вы действительно хотите отказаться от вознесения? Это действие нельзя отменить.", "Пир Сов", list("Да, я уверен", "Нет"), 30 SECONDS)
	...
	for(var/i in 1 to reward)
		...
		to_chat(user, span_danger("Вы чувствуете, как нечто невидимое разрывает саму вашу сущность!"))
		user.do_jitter_animation()
		sleep(1 SECONDS)
		if(QDELETED(user) || QDELETED(heretic_datum))
			return FALSE

	to_chat(user, span_danger(span_big("Ваши амбиции растерзаны, но после них осталось нечто могущественное...")))
