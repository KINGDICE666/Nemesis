...
/obj/machinery/atmospherics/components/unary/vent_pump
	...
	name = "воздушная вентиляция"
	desc = "К ней присоединены клапан и насос."
...
/obj/machinery/atmospherics/components/unary/vent_pump/examine(mob/user)
	. = ..()
	. += span_notice("Ее можно связать с воздушным сенсором с помощью мультитула.")

	if(fan_overclocked)
		. += span_warning("Сейчас она разогнана и со временем получает повреждения.")
...
/obj/machinery/atmospherics/components/unary/vent_pump/welder_act(mob/living/user, obj/item/welder)
	...
	if(welder.use_tool(src, user, 20, volume=50))
		if(!welded)
			user.visible_message(span_notice("[user] заваривает вентиляцию."), span_notice("Вы завариваете вентиляцию."), span_hear("Вы слышите сварку."))
			welded = TRUE
		else
			user.visible_message(span_notice("[user] срезает сварку с вентиляции."), span_notice("Вы срезаете сварку с вентиляции."), span_hear("Вы слышите сварку."))
			welded = FALSE
		update_appearance(UPDATE_ICON)
...
/obj/machinery/atmospherics/components/unary/vent_pump/can_unwrench(mob/user)
	...
	if(. && on && is_operational)
		to_chat(user, span_warning("Нельзя открутить [src], сначала выключите ее!"))
		return FALSE
...
/obj/machinery/atmospherics/components/unary/vent_pump/attack_alien(mob/user, list/modifiers)
	if(!welded || !(do_after(user, 2 SECONDS, target = src)))
		return
	user.visible_message(span_warning("[user] яростно скребет [src]!"), span_notice("Вам удается убрать то, что блокировало вентиляцию."), span_hear("Вы слышите громкий скрежет."))
	welded = FALSE
	update_appearance(UPDATE_ICON)
	pipe_vision_img = image(src, loc, dir = dir)
	SET_PLANE_EXPLICIT(pipe_vision_img, ABOVE_HUD_PLANE, src)
...
/obj/machinery/atmospherics/components/unary/vent_pump/high_volume
	name = "большая воздушная вентиляция"
