...
/obj/machinery/atmospherics/miner
	name = "газовый добытчик"
	desc = "Газы, добытые из газового гиганта снизу (или сверху?), выходят через эту массивную вентиляцию."
...
/obj/machinery/atmospherics/miner/examine(mob/user)
	. = ..()
	if(broken)
		. += {"Его отладочный вывод показывает: "[broken_message]"."}
...
/obj/machinery/atmospherics/miner/proc/check_operation()
	...
	if(OT.planetary_atmos)
		broken_message = span_boldwarning("УСТРОЙСТВО НЕ ЗАКЛЮЧЕНО В ГЕРМЕТИЧНУЮ СРЕДУ")
		set_broken(TRUE)
		return FALSE
	if(isspaceturf(T))
		broken_message = span_boldnotice("ВОЗДУХ УХОДИТ В КОСМОС")
		set_broken(TRUE)
		return FALSE
	...
	if(G.total_moles() > max_ext_mol)
		broken_message = span_boldwarning("ВНЕШНЯЯ КОНЦЕНТРАЦИЯ ГАЗА ВЫШЕ ПОРОГА")
		set_broken(TRUE)
		return FALSE
	if(broken)
		set_broken(FALSE)
