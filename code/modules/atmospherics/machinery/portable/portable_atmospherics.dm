...
/obj/machinery/portable_atmospherics
	name = "портативное атмосферное устройство"
...
/obj/machinery/portable_atmospherics/examine(mob/user)
	. = ..()
	if(nob_crystal_inserted)
		. += "Внутри установлен кристалл гиперноблия, позволяющий подавлять реакции."
	if(suppress_reactions)
		. += "Кристалл гиперноблия внутри слабо светится синим, показывая, что реакции сейчас подавляются."
...
/obj/machinery/portable_atmospherics/welder_act(mob/living/user, obj/item/tool)
	...
	while(tool.use_tool(src, user, 2.5 SECONDS, volume=40))
		...
		balloon_alert(user, "частично отремонтировано...")
...
/obj/machinery/portable_atmospherics/click_alt(mob/living/user)
	...
	to_chat(user, span_notice("Вы извлекаете [holding] из [src]."))
...
/obj/machinery/portable_atmospherics/examine(mob/user)
	...
	if(!holding)
		return
	. += span_notice("В [src] находится [holding]. Alt-click по [src], чтобы извлечь его.")+\
		span_notice("Кликните по [src] другим газовым баллоном, чтобы быстро заменить [holding].")
...
/obj/machinery/portable_atmospherics/proc/replace_tank(mob/living/user, close_valve, obj/item/tank/new_tank)
	...
	if(holding && new_tank)//for when we are actually switching tanks
		...
		to_chat(user, span_notice("Одним плавным движением вы вынимаете [holding] из разъема [src] и заменяете его на [new_tank]."))
		...
		to_chat(user, span_notice("Вы вставляете [new_tank] в [src]."))
		holding = new_tank
		playsound(src, insert_sound, sound_vol)
		RegisterSignal(holding, COMSIG_QDELETING, PROC_REF(unregister_holding))
...
/obj/machinery/portable_atmospherics/wrench_act(mob/living/user, obj/item/wrench)
	...
	if(connected_port)
		...
		user.visible_message( \
			...
			span_notice("Вы открепляете [src] от порта."), \
	...
	if(!connect(possible_port))
		to_chat(user, span_notice("[name] не удалось подключиться к порту."))
		return FALSE
	wrench.play_tool_sound(src)
	user.visible_message( \
		"[user] подключает [src].", \
		span_notice("Вы закрепляете [src] на порту."), \
		span_hear("Вы слышите трещотку."))
	update_appearance()
