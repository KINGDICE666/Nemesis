/obj/machinery/power/apc/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	if(isAI(user) || iscyborg(user))
		...
		context[SCREENTIP_CONTEXT_SHIFT_LMB] = lighting ? "Отключить свет" : "Включить свет"
		context[SCREENTIP_CONTEXT_ALT_LMB] = equipment ? "Отключить оборудование" : "Включить оборудование"
		context[SCREENTIP_CONTEXT_CTRL_SHIFT_LMB] = environ ? "Отключить жизнеобеспечение" : "Включить жизнеобеспечение"
		...
		if (opened == APC_COVER_CLOSED)
			context[SCREENTIP_CONTEXT_LMB] = "Открыть крышку"
			...
			context[SCREENTIP_CONTEXT_LMB] = "Извлечь поврежденную плату"
		else
			context[SCREENTIP_CONTEXT_LMB] = "Извлечь плату"

	else if(held_item.tool_behaviour == TOOL_SCREWDRIVER)
		if (opened == APC_COVER_CLOSED)
			context[SCREENTIP_CONTEXT_LMB] = panel_open ? "Скрыть провода" : "Открыть провода"
		...
		if (terminal && opened == APC_COVER_OPENED)
			context[SCREENTIP_CONTEXT_LMB] = "Разобрать кабельный терминал"

	else if(held_item.tool_behaviour == TOOL_WELDER)
		if (opened == APC_COVER_OPENED && !has_electronics)
			context[SCREENTIP_CONTEXT_LMB] = "Разобрать APC"
		...
		context[SCREENTIP_CONTEXT_LMB] = "Создать кабельный терминал"
		...
		if (!has_electronics)
			context[SCREENTIP_CONTEXT_LMB] = "Вставить плату APC"
		else if(!cell)
			context[SCREENTIP_CONTEXT_LMB] = "Вставить батарею"

	else if(istype(held_item, /obj/item/wallframe/apc))
		context[SCREENTIP_CONTEXT_LMB] = "Заменить поврежденную раму"
