...
/datum/sm_delam/proc/delam_progress(obj/machinery/power/supermatter_crystal/sm)
	...
	if(sm.damage >= sm.emergency_point) // In emergency
		sm.radio.talk_into(sm, "ДЕЛАМИНАЦИЯ КРИСТАЛЛА НЕИЗБЕЖНА! Целостность: [round(sm.get_integrity_percent(), 0.01)]%", sm.emergency_channel)
		...
		sm.radio.talk_into(sm, "Опасность! Целостность кристаллической гиперструктуры падает! Целостность: [round(sm.get_integrity_percent(), 0.01)]%", sm.warning_channel)
...
/datum/sm_delam/proc/count_down_messages(obj/machinery/power/supermatter_crystal/sm)
	...
	messages += "ДЕЛАМИНАЦИЯ КРИСТАЛЛА НЕИЗБЕЖНА. Суперматерия достигла критического отказа целостности. Аварийное поле дестабилизации причинности активировано."
	messages += "Кристаллическая гиперструктура возвращается к безопасным рабочим параметрам. Аварийная защита отключена."
	messages += "остаётся до стабилизации причинности."
	return messages
