...
/proc/sm_gas_data()
	...
	for (var/gas_path in GLOB.sm_gas_behavior)
		...
		if(sm_gas.heat_power_generation)
			...
			numeric_data += list(list(
				"name" = "Прирост энергии от нагрева",
				"amount" = si_derived_data["coefficient"],
				"unit" = si_derived_data["unit"],
				"positive" = TRUE,
			))
		if(sm_gas.powerloss_inhibition)
			numeric_data += list(list(
				"name" = "Снижение рассеивания энергии",
...
/datum/sm_gas/carbon_dioxide
	...
	desc = "При поглощении суперматерией и контакте с кислородом будет производиться плюоксиум."
...
/datum/sm_gas/bz
	...
	heat_power_generation = 1
	desc = "Будет испускать ядерные частицы при концентрации выше 40%."
...
/datum/sm_gas/miasma
	...
	desc = "Будет поглощаться суперматерией для выработки энергии."
...
/datum/sm_gas/miasma/extra_effects(obj/machinery/power/supermatter_crystal/sm)
	...
	sm.log_activation("поглощением миазмы")
...
/datum/sm_gas/zauker
	...
	heat_power_generation = 1
	desc = "Будет создавать электрические разряды."
