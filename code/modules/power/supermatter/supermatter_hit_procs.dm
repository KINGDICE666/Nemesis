...
/obj/machinery/power/supermatter_crystal/proc/eat_bullets(datum/source, obj/projectile/projectile)
	...
	if(projectile.armor_flag != BULLET || kiss_power)
		...
		if(damage_to_be > danger_point)
			visible_message(span_notice("[src] сжимается под нагрузкой, сопротивляясь дальнейшим попаданиям!"))
...
/obj/machinery/power/supermatter_crystal/singularity_act()
	...
	for(var/mob/hearing_mob as anything in GLOB.player_list)
		...
		to_chat(hearing_mob, span_bolddanger("Ужасный визг наполняет ваши уши, и вас накрывает волна страха..."))
	qdel(src)
	return gain
...
/obj/machinery/power/supermatter_crystal/attack_tk(mob/user)
	...
	to_chat(jedi, span_userdanger("Это была действительно плотная мысль."))
...
/obj/machinery/power/supermatter_crystal/attackby(obj/item/item, mob/user, list/modifiers, list/attack_modifiers)
	if(istype(item, /obj/item/scalpel/supermatter))
		...
		to_chat(user, span_notice("Вы осторожно начинаете скоблить \the [src] с помощью \the [scalpel]..."))
		if(!scalpel.use_tool(src, user, 60, volume=100))
			return
		if (scalpel.usesLeft)
			to_chat(user, span_danger("Вы извлекаете осколок из \the [src]. \The [src] начинает бурно реагировать!"))
			...
			scalpel.usesLeft--
			if (!scalpel.usesLeft)
				to_chat(user, span_notice("Крошечная часть \the [scalpel] отламывается, делая его бесполезным!"))
		else
			to_chat(user, span_warning("Вам не удаётся извлечь осколок из \the [src]! \The [scalpel] уже недостаточно острый."))
		return
	...
	if(istype(item, /obj/item/destabilizing_crystal))
		...
		if(!is_main_engine)
			to_chat(user, span_warning("Нельзя использовать \the [destabilizing_crystal] на \a [name]."))
			return

		if(get_integrity_percent() < SUPERMATTER_CASCADE_PERCENT)
			to_chat(user, span_warning("\The [destabilizing_crystal] можно применить только к \a [name] с целостностью не ниже [SUPERMATTER_CASCADE_PERCENT]%."))
			return

		to_chat(user, span_warning("Вы начинаете прикреплять \the [destabilizing_crystal] к \the [src]..."))
		if(do_after(user, 3 SECONDS, src))
			...
			to_chat(user, span_danger("\The [destabilizing_crystal] защёлкивается на \the [src]."))
