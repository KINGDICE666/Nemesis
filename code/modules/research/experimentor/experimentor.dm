/obj/machinery/rnd/experimentor
	...
	desc = "Экспериментальный ксеноновый энтропийный реакционный инфузор частиц, или что-то вроде того. Новый реакционный инфузор Nanotrasen, который чуть реже катастрофически выходит из строя, чем предыдущая модель... по крайней мере, так говорят."
...
/obj/machinery/rnd/experimentor/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		...
		. += span_notice("Вероятность сбоя снижена на [span_bold("[malfunction_probability_coeff]")].")
		. += span_notice("Интервал перезарядки между экспериментами: [span_bold("[cooldown]")] секунд.")
...
/obj/machinery/rnd/experimentor/proc/item_eject(should_clone = FALSE)
	...
	if(should_clone)
		visible_message(span_notice("Из [src] выскакивает копия \the [loaded_item]!"))
...
/obj/machinery/rnd/experimentor/proc/handle_global_reactions()
	...
	switch(malf_coefficient)
		if(1 to 15)
			visible_message(span_warning("Бортовая система обнаружения [src] дала сбой!"))
			item_reactions["[loaded_item.type]"] = pick(get_available_reactions())
			item_eject()
		if(16 to 35)
			visible_message(span_warning("[src] расплавляет [loaded_item], странно меняя воздух вокруг!"))
			do_smoke(1, src, src)
		...
		if(36 to 50)
			visible_message(span_warning("[src] вытягивает жизненную силу из всех поблизости!"))
			for(var/mob/living/m in view(4,src))
				to_chat(m, span_danger("Вы чувствуете, как вашу плоть разрывает, а кровавая дымка тянется к [src]!"))
		...
		if(51 to 75)
			visible_message(span_warning("[src] сталкивается с ошибкой выполнения!"))
			do_smoke(1, src, src)
		...
		if(76 to 98)
			visible_message(span_warning("[src] издает низкий гул."))
		...
		if(99)
			visible_message(span_warning("[src] начинает светиться и вибрировать. Сейчас рванет!"))
...
/obj/machinery/rnd/experimentor/item_interaction(mob/living/user, obj/item/weapon, list/modifiers)
	...
	if(!user.transferItemToLoc(weapon, src))
		to_chat(user, span_warning("\The [weapon] прилип к вашей руке!"))
		return ITEM_INTERACT_BLOCKING

	loaded_item = weapon
	to_chat(user, span_notice("Вы помещаете [weapon] в камеру."))
