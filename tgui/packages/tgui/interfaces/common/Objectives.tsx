...
export const ObjectivePrintout = (props: ObjectivePrintoutProps) => {
  ...
  return (
    <Stack fill={fill} vertical>
      <Stack.Item bold>{titleMessage || `Ваши текущие цели`}:</Stack.Item>
      <Stack.Item>
        {(objectives.length === 0 && 'Нет!') ||
...
export const ReplaceObjectivesButton = (props: ReplaceObjectivesProps) => {
  const {
    ...
    button_title,
    button_colour,
    button_tooltip = 'Заменить текущие цели на пользовательскую. Это действие можно выполнить только один раз',
  } = props;
